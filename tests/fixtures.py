...
_DEMO_DATASETS = [
    (
        ...
        "city-portland",
        (-122.75, 45.45, -122.55, 45.60),  # within Portland, OR
    ),
    (
        "portland-park-trees",
        ...
        "city-portland",
        (-122.70, 45.48, -122.60, 45.55),  # within Portland, OR
    ),
    (
        ...
        "state-california",
        (-122.0, 37.0, -119.0, 39.0),  # within California
    ),
    (
        ...
        "state-california",
        (-121.0, 36.5, -120.0, 38.0),  # within California
    ),
    (
        ...
        "county-cook",
        (-87.95, 41.65, -87.55, 42.05),  # within Cook County, IL
    ),
    (
        ...
        "univ-michigan",
        (-83.80, 42.22, -83.68, 42.32),  # within Ann Arbor, MI
    ),
    (
        ...
        "univ-michigan",
        None,  # non-spatial, so the non-geospatial filter has variety too
    ),
    (
        "navajo-water-access",
        ...
        "tribe-navajo",
        (-110.5, 35.5, -109.0, 37.0),  # within Navajo Nation region
    ),
    (
    ...
    (
        ...
        "American Red Cross",
        "nonprofit-redcross",
        (-118.50, 33.90, -118.10, 34.20),  # within Los Angeles, CA
    ),
]

# Demo locations whose geometry covers the demo datasets above, so selecting one
# in the geography filter returns those datasets.
...
def _add_filter_demo_data(fixture_dict):
    ...
    # Each organization needs at least one harvest source for its datasets.
...
def fixture_data():
    ...
    # Extra seed data so the organization, organization-type, publisher, and
    # geography filters all have variety to exercise locally. These records are
    # purposely kept separate from the records above (and from the americorps
    # CSV, which all belong to org "1") so existing count-based tests still hold.
