...
def test_index_shows_suggested_keywords_when_no_keywords_selected(
    interface_with_dataset, db_client
):
    """Test that suggested keywords are shown when no keywords are selected."""
    with patch("app.routes.interface", interface_with_dataset):
        response = db_client.get("/")
    ...
    # Check that suggested keywords container exists
    suggested_container = soup.find("div", {"id": "suggested-keywords"})
    assert suggested_container is not None

    # Check that suggested keywords are present as buttons
    suggested_buttons = suggested_container.find_all(
        "button", class_="tag-link--suggested"
    )
    assert len(suggested_buttons) > 0

    # Check that each button has the data-keyword attribute
    for button in suggested_buttons:
        assert button.get("data-keyword") is not None
        assert button.get("type") == "button"


def test_index_hides_suggested_keywords_when_keyword_selected(
    interface_with_dataset, db_client
):
    """Test that suggested keywords are hidden when a keyword is selected."""
    with patch("app.routes.interface", interface_with_dataset):
        # Request with a keyword parameter
        response = db_client.get("/?keyword=health")

    assert response.status_code == 200
    soup = BeautifulSoup(response.text, "html.parser")

    # Suggested keywords container should not be present when keywords are selected
    suggested_container = soup.find("div", {"id": "suggested-keywords"})
    assert suggested_container is None
